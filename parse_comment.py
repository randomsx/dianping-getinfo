...
def process_cnum(number, css_result, num_dict):
    ...
    处理数字
    """
    items = re.findall('<d class="(.*?)"></d>', number, re.S)  # 这里目前样式是d class=
    number = re.sub('</b>', '', re.sub('<b>', '', number))  # 这行无用
    for item in items:
        if item in css_result:
            ...
            for num in list(num_dict.keys()):
                if int(y) > int(num):
                    ...
                    number = re.sub('<d class="%s"></d>' % item, real_num, number)
    return number


def process_phone_number(fake_num):
    numbers = re.sub('<d class="num">&#x', ' ', re.sub(';</d>', ' ', fake_num.split("</span>")[1]))
    # print("=====")
    # print(numbers)
    # print("=====")
    ...
    phone_number = ''
    phone_number_dict = {
        'efeb': '1', 'e4ff': '2', 'f70d': '3', 'e6ec': '4', 'f404': '5', 'e65d': '6', 'e284': '7',
        'f810': '8', 'e27b': '9', 'ec2d': '0', '&nbsp;': ' ', '</p>': ''
    }
